...
public class PLYMeshReader implements MeshReader {
	...
	public HashSet<Polygon> read(String filename) throws WrongFileFormatException {
		...
		// initialise regex array to check header
		Pattern[] headerRegexes = {
				Pattern.compile("ply"),
		...
		// if code reached this point, we know the header is right
		// get number of vertices and polygons from the header
		String[] headerTokens = lines.get(2).split(" ");
		int expectedVertices = Integer.valueOf(headerTokens[2]);
		headerTokens = lines.get(6).split(" ");
		int expectedPolygons = Integer.valueOf(headerTokens[2]);
		
		
		// initialise regexes for the vertices and faces
		Pattern vertexPattern = Pattern.compile("\\d+(\\.\\d+)? +\\d+(\\.\\d+)? +\\d+(\\.\\d+)?");
